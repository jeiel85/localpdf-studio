...
이 문서는 AI 에이전트가 처리할 수 없는 외부 계정/인증 기반 작업만 모아둔다. 저장소 코드, v0.18.0 GitHub Release, Windows/macOS/Linux 산출물, updater `latest.json`, 패키지 매니페스트 동기화, CI 검증은 완료되어 있다.

## 현재 완료 상태

- v0.18.0 GitHub Release 완료: <https://github.com/jeiel85/localpdf-studio/releases/tag/v0.18.0>
- 릴리즈 산출물 17개 업로드 및 검증 완료
- `latest.json` updater URL 검증 완료
- Homebrew Cask 로컬 매니페스트 v0.18.0 SHA 반영 완료
- winget 공식 소스는 현재 `0.17.0` 노출, v0.18.0 PR 제출 대기
- Chocolatey v0.18.0 패키지 메타데이터 반영 완료, community push 대기
- Snap/AUR/Homebrew/winget/Chocolatey 매니페스트는 v0.18.0 SHA 기준으로 동기화 완료

## 사용자가 직접 해야 하는 작업

### 1. winget PR 리뷰/CLA 처리

- 현재 상태: v0.18.0 매니페스트 SHA 반영 완료, PR 제출 대기
- 해야 할 일:
  - `scripts/windows/submit-winget.ps1 -Version 0.18.0` 또는 `wingetcreate submit`으로 새 PR 제출
...
- 저장소 준비 상태:
  ...
  - v0.18.0 community push는 아직 미실행

- 현재 상태: v0.18.0 패키지 메타데이터 반영 완료, community push 대기. 기존 v0.17.2는 `choco search localpdf-studio --exact --source https://community.chocolatey.org/api/v2/` 기준으로 아직 검색 미노출.
- 해야 할 일:
  - `scripts/windows/publish-chocolatey.ps1`로 v0.18.0 push
...
gh release view v0.18.0 --repo jeiel85/localpdf-studio --json url,assets
