...
| 매니저 | 명령어 | 상태 |
|---|---|---|
| <img src="https://cdn.simpleicons.org/windowsterminal/4D4D4D" width="14" height="14" /> **winget** | `winget install jeiel85.LocalPDFStudio` | v0.17.0 PR 제출 완료 ([microsoft/winget-pkgs#377207](https://github.com/microsoft/winget-pkgs/pull/377207)) |
| <img src="https://cdn.simpleicons.org/chocolatey/80B5E3" width="14" height="14" /> **Chocolatey** | `choco install localpdf-studio` | v0.17.0 `.nupkg` 생성 검증 완료, `CHOCO_API_KEY` 주입 후 push |
| <img src="https://cdn.simpleicons.org/snapcraft/82BEA0" width="14" height="14" /> **Snap** | `sudo snap install localpdf-studio` | v0.15.0 snapcraft.yaml 반영 완료, v0.17.2 Linux 태그 빌드 후 갱신 |
| <img src="https://cdn.simpleicons.org/archlinux/1793D1" width="14" height="14" /> **AUR** | `yay -S localpdf-studio-bin` | v0.15.0 PKGBUILD/SHA 반영 완료, v0.17.2 Linux 태그 빌드 후 갱신 |
| <img src="https://cdn.simpleicons.org/homebrew/FBB040" width="14" height="14" /> **Homebrew** | `brew tap jeiel85/tap && brew install --cask localpdf-studio` | Cask 작성 완료, v0.17.2 macOS 태그 빌드 후 personal tap 갱신 |
...
- **배포 자동화** — 태그 기반 Windows 릴리즈 빌드, GitHub Release, Tauri updater manifest 자동 생성.
...
### 🖊️ PDF Fill & Sign (v0.17.0 New!)
- **자유 스탬프 도구** — PDF 위 어디에든 텍스트, ✓, ✕, ●, 날짜를 드래그/클릭만으로 배치, 모서리 핸들로 자유 리사이즈, 더블 클릭 없이 즉시 이동.
- **두 가지 서명 방식** — 마우스/스타일러스로 직접 그리는 캔버스 서명, 또는 이미지(PNG/JPG) 파일에서 가져온 서명을 흰 배경 자동 투명화로 처리해 임포트.
- **서명 라이브러리** — 그린 서명과 가져온 서명을 localStorage에 영속 저장해 여러 PDF에 재사용. 즉시 미리보기 카드로 정렬.
- **AcroForm 폼 평탄화 (Flatten)** — 폼 채우기 후 필드를 평탄화해 편집 불가능한 정적 PDF로 고정. Fill & Sign 저장 시에도 함께 평탄화 가능.
- **회전 대응 좌표계** — 0°/90°/180°/270° 회전과 모든 줌 배율에서도 unrotated PDF Point 기준으로 정확한 위치 유지.

### ✒️ 주석 및 보안 마스킹 (v0.15.0)
...
- 범용 파일 저장/읽기/삭제 command 허용 확장자 제한 및 임시 파일 기반 교체 저장
...
- **패키지 매니저 매니페스트 동기화 자동화** — GitHub Release asset digest를 읽어 winget, Chocolatey, Homebrew, Snap, AUR 제출 파일의 버전/URL/SHA-256을 한 번에 갱신
...
scripts/windows/             Windows 우클릭 메뉴 + Portable ZIP 생성 스크립트
  └── sync-package-manifests.ps1  패키지 매니저 매니페스트 버전/SHA 동기화
